# -*- coding: utf-8 -*-
"""
Presto MAX — Job Cost Calculator
...
# -------------------------------------------------------------------------
# Helpers para ROI / Payback (Lógica Financeira)
# -------------------------------------------------------------------------
...
def compute_payback_schedule(
    ...
    for month in range(1, horizon + 1):
        # Fator de rampa (eficiência inicial)
        if ramp_m <= 1:
        ...
        # Fator de crescimento pós-rampa
...
PRINT_MODES: Dict[str, Dict[str, object]] = {
    "Fast Quality":         {"speed": 270.0, "res_color": "800×400"},
    "Fast Production":      {"speed": 475.0, "res_color": "800×400"},
    "Standard Quality":     {"speed": 180.0, "res_color": "600×800"},
    "Standard Production":  {"speed": 278.0, "res_color": "600×800"},
    "Saturation Quality":   {"speed": 115.0, "res_color": "1000×800"},
    "Saturation Production":{"speed": 210.0, "res_color": "1000×800"},
}
...
def ml_per_m2_from_xml_bytes(xml_bytes: bytes) -> Dict[str, float]:
    """Return ml/m² per separation from XML bytes."""
...
def parse_controlcenter_pdf(pdf_bytes: bytes) -> Dict[str, Any]:
    ...
    - ml_map_m2: dict channel -> ml/m² (Fixation mapped separately)
    ...
    # Size, examples:
    # 'Tamaño 1.74 m x 0.20 m'  or  'Tamano 1.74 m x 0.20 m'
    width_m = 0.0
    length_m = 0.0
    m_size = re.search(
    ...
    # Ink table line – ml/m² row
    ml_map_m2: Dict[str, float] = {}

    pattern_ml = r"ml\s*/\s*m[²2]?\s+((?:[\d.,]+\s+){3,}[\d.,]+)"
...
def compute_full_job(
    ...
    # Laser cutting – detailed cost (labour + energy + depreciation)
...
def main() -> None:
    st.set_page_config(
        page_title="Presto MAX — Job Cost Calculator",
        page_icon="🧵",
        layout="wide",
    )
    ...
    with header_col1:
        try:
            ...
            st.write("")
    with header_col2:
        st.title("Presto MAX — Job Cost Calculator")
        st.caption(
            "Job cost calculator for a specific order (roll) on the Presto MAX printer."
        )

    # Global reset button – clears all inputs and calculations
    ...
    with tab_conf:
        ...
        consumption_source = col_source.radio(
            ...
            ["Machine XML inside ZIP", "Manual (ml/m²)", "ControlCenter PDF report"],
            index=0,
            horizontal=True,
        )
        col_source.caption("👉 Upload the XML, manual data or ControlCenter PDF in the **Ink consumption** tab.")
        ...
        other_var_simple_per_m2 = col_fab2.number_input(
            f"Other variable costs ({currency_label()}/m²)",
            min_value=0.0,
            value=0.0,
        ...
        st.markdown("---")
        st.subheader("Laser cutting – detailed parameters")
        ...
        laser_power_kw = col_l4.number_input(
            ...
            min_value=0.0,
            value=0.0,
            help="Typical small laser cutter: 1.0–3.0 kW, big units can be higher.",
        )
        col_l5.caption(
            "Laser cutting cost = labour (people × hours × salary) "
            "+ laser energy (kW × electricity price × hours)."
        )
        ...
        st.markdown("---")
        st.subheader("Equipment – energy & depreciation (Presto MAX)")
        ...
        st.markdown("---")
        st.subheader("Labour (monthly salary → cost/h)")
    ...
    with tab_laser_price:
        ...
        with st.expander("⚙️ Configure hourly costs (labour, energy, machine depreciation)", expanded=False):
            st.info(
            ...
            [
                {
                    ...
                    "Effective area (m², incl. waste)": area_laser_effective,
        ...
        st.markdown("**Cost summary – independent laser job**")
        ...
        st.markdown("---")
        st.markdown("**Pricing scenarios – independent laser job**")
        ...
        with col_tbl_ind:
            st.markdown("**Table – Suggested prices for independent laser job**")
        ...
        st.markdown("**User-defined price and real margin – independent laser job**")
        ...
        col_lu3.metric(
            "Real profit margin (%) – independent laser job",
            f"{margin_laser_user_pct:.2f}%",
        )

        st.markdown("**Cost vs selling price vs profit – independent laser job**")
        ...
        ax_lcp.set_title("Unit economics – independent laser service")
        ...
        st.download_button(
            "📥 Download laser service Excel report",
    ...
    with tab_ink:
        ...
        if consumption_source.startswith("Machine"):
            ...
            st.subheader("Manual consumption (ml/m²)")
            col_c, col_w, col_f = st.columns(3)
            man_color = col_c.number_input("Color (ml/m²)", min_value=0.0, value=8.0)
            man_white = col_w.number_input("White (ml/m²)", min_value=0.0, value=0.0)
            man_fof = col_f.number_input(
                "FOF / pretreatment (ml/m²)", min_value=0.0, value=3.0
            )
                ...
                if up_pdf is not None:
                    ...
                    try:
                        ...
                        if not ml_map_m2:
                            ...
                            with col_pdf_tbl:
                                st.markdown(
                                    "**Table – Ink consumption per channel (ml/m², from PDF)**"
                                )
    ...
    with tab_results:
        ...
        if not calc:
            ...
            with col_cost_tbl:
                st.markdown("**Table – Cost by category (total job)**")
            ...
            with col_pct_tbl:
                st.markdown("**Table – Share of total cost (%)**")
            ...
            df_summary = pd.DataFrame(
                [
                    {
                        "Printed area (m², incl. waste)": res["area_with_waste"],
                        ...
                        "Cost per m²": cost_per_m2_with_energy,
    ...
    with tab_price:
        ...
        if not calc:
            ...
            base_unit = st.radio(
                ...
                ["Per m²", "Per linear metre"],
                index=1,
                horizontal=True,
            )
            if base_unit.startswith("Per m²"):
                base_cost = cost_per_m2
                base_label = f"Selling price per m² ({currency_label()})"
                profit_label = f"Profit per m² ({currency_label()})"
            else:
            ...
            if base_cost <= 0:
                ...
                with col_tbl:
                    st.markdown("**Table – Suggested prices by margin**")
                ...
                st.markdown("---")
                st.subheader("C. User-defined selling price – real margin")
                ...
                st.markdown("**Unit economics – cost vs price vs profit**")
                ...
                ax_u.set_title("Unit economics – selected base unit")
    ...
    with tab_roi:
        ...
        st.caption("Projeção de fluxo de caixa mensal e payback. Defina custos variáveis (detalhados), custos fixos (detalhados) e investimento.")

        sym = currency_label()
        
        # --- RECUPERAÇÃO DE DADOS (do Tab 3: Cost Results) ---
        calc = st.session_state.get("calc")
        ...
        if calc:
            # Tenta pegar depreciação calculada no Tab 3
        ...
        with econ_col:
            st.markdown("**Unit Economics (Economia Unitária)**")
            
            unit_label_roi = st.selectbox("Unit Type", ["Linear Meter (m)", "Square Meter (m²)", "Piece"], index=0)
            ...
            price_input = st.number_input(
                ...
                step=0.50,
                format="%.2f",
                help="Preço de venda final por unidade."
            )
            
            # 2. CUSTOS VARIÁVEIS (DETALHADO vs TOTAL)
            st.markdown("---")
            ...
            if var_mode == "Detailed Breakdown":
                ...
                ink_cons_ml = col_ink_cons.number_input("Consumption (ml/m²)", value=float(def_ink_cons_m2), min_value=0.0, format="%.2f")
                
                # Custo tinta por m²
                v_ink_m2 = (ink_price_l / 1000.0) * ink_cons_ml
                
                # --- 2.2 Mídia (Input em $/lm) ---
                ...
                # --- Conversão para Unit Type selecionado ---
            ...
            if fix_mode == "Detailed (Monthly Helper)":
                ...
                depreciation_monthly_val = st.number_input(f"Of which is Depreciation {sym}", value=1000.0, step=100.0, help="Valor da depreciação incluso no total acima.")
        ...
        with invest_col:
            ...
            machine_investment = st.number_input(
                ...
                step=1000.0,
                help="Valor total do investimento (deve bater com o valor usado para depreciação se for o mesmo ativo)."
            )
        ...
        if monthly_units_input <= 0:
            ...
            # Custo Fixo "Cash" (Exclui depreciação para cálculo de fluxo de caixa)
            fixed_cash_outflow = max(0.0, total_fixed_monthly - depreciation_monthly_val)
            
            # Fluxo de Caixa Estável (Sem Rampa)
            ...
            m2.metric("Steady Monthly Net Cash", pretty_money(steady_cash, sym), help="Fluxo de caixa mensal líquido após a fase de rampa (EBITDA proxy).")
            
            if payback_month:
                m3.metric("Payback (months)", f"{payback_month}")
                m3.caption(f"≈ {payback_date:%b %Y}")
            else:
    ...
    with tab_break:
        ...
        if not calc:
            ...
            if area > 0:
                ...
                rows.append({"Channel Group": "Color Inks", "ml/m²": res.get("color_ml_per_m2", 0.0), f"Total Cost ({sym})": color_cost_total})
                # White ink
                white_ml_total = res.get("white_ml_per_m2", 0.0) * area
                white_cost_total = (white_ml_total / 1000.0) * ink_white_per_l
                rows.append({"Channel Group": "White Ink", "ml/m²": res.get("white_ml_per_m2", 0.0), f"Total Cost ({sym})": white_cost_total})
                # FOF
                fof_ml_total = res.get("fof_ml_per_m2", 0.0) * area
                fof_cost_total = (fof_ml_total / 1000.0) * fof_per_l
                rows.append({"Channel Group": "FOF/Pretreatment", "ml/m²": res.get("fof_ml_per_m2", 0.0), f"Total Cost ({sym})": fof_cost_total})
